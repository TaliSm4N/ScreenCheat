...
void compareID(char *id, char *pwd,struct profile *Upf,int * sid) // ID, PWD를 비교한다. 일치하는 정보가 없다면 sid = -1(틀리는것도 해당)
{ // sid는 단순히 대입하기 위해서 가져옴 
	runQuery("Select * from Userinfo"); // 비교대상을 가져옴

	while(fetchRow()) // 검사 진행, 유저를 하나씩 가져와서 대조하는 부분
	{// 유저 수 이상으로 fetchRow를 하면 아래 if문에서 segmentation이 나오는 상황(없는 값들을 참조하려 하니까)
